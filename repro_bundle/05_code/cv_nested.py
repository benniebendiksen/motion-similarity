...
def main():
    ...
    ap.add_argument("--ckpt-stride", type=int, default=3)     # legacy; unused under grid design
    ap.add_argument("--epoch-grid", type=int, default=100,
                    help="UNIFORM epoch grid for candidate checkpoints (default 100 = LCM of save "
                         "intervals). Every grid checkpoint <= loss-plateau is an inner-loop "
                         "candidate; per-fold SELECT picks among them. Applied identically to all "
                         "models (the non-cherry-pick rule).")
    ap.add_argument("--loss-plateau", type=int, default=None,
                    help="per-model loss-plateau epoch = upper bound of the candidate range "
                         "[0, loss_plateau]. If None, uses the full saved trajectory.")
    ...
    ap.add_argument("--ckpt-epochs", nargs="+", type=int, default=None,
                    help="explicit candidate epochs (overrides the grid; legacy / debugging)")
    ...
    for enc in args.encoders:
        ...
        # FINAL DESIGN (2026-06-21): epoch is an inner-loop hyperparameter over a UNIFORM epoch
        # grid across [0, loss_plateau]. Every grid checkpoint is a candidate (NOT a hand-picked
        # 7); the per-fold SELECT step chooses among them and TEST reports. PSP is a diagnostic
        # only, not used here. See memory project_architecture_loss_sweep.
        if args.ckpt_epochs:                              # explicit override (legacy / debugging)
            cks = [c for c in cv.list_checkpoints(enc, 1) if c[0] in args.ckpt_epochs]
        else:
            cks = [c for c in cv.list_checkpoints(enc, 1)
                   if c[0] > 0 and c[0] % args.epoch_grid == 0
                   and (args.loss_plateau is None or c[0] <= args.loss_plateau)]
        ts(f"  {len(cks)} candidate checkpoints (grid={args.epoch_grid}, "
           f"plateau={args.loss_plateau}): {[e for e,_ in cks]}")

        # ---- PRE-ENCODE CACHE: embeddings are FOLD-INDEPENDENT, so encode each candidate x
        # action exactly ONCE and reuse across all repeats*folds (kills the ~25x redundant
        # re-encode). cache_dir[(ep, action)] -> emb dir.
        cache = {}
        ts(f"  pre-encoding {len(cks)} candidates x {len(ACTIONS)} actions (once) ...")
        for ep, path in cks:
            for a in ACTIONS:
                od = os.path.join(args.work_dir, f"cache_{enc}_ep{ep}_{a}")
                if not (os.path.isdir(od) and glob.glob(os.path.join(od, "*_emb.pt"))):
                    cv.encode(enc, path, a, od)
                cache[(ep, a)] = od
        ts(f"  cache ready: {len(cache)} (epoch,action) embedding dirs")
        fold_rows = []
        for rep in range(args.repeats):
          ...
          for f in range(args.k):
            ...
            # reads CACHED embeddings (encoded once above); no per-fold re-encode.
            best = (None, -2.0, None)
            for ep, _ in cks:
                per = {a: cv.raw_spearman(cache[(ep, a)], a, select[a], modules[a]) for a in ACTIONS}
                vals = [v for v in per.values() if v == v]
                ms = float(np.mean(vals)) if vals else -2.0
                if ms > best[1]:
                    best = (ep, ms, per)
            sel_ep, sel_meanS, sel_per = best
            ts(f"[{tag}] {enc} selected ep={sel_ep} (SELECT mean-S={sel_meanS:.3f} {({a:round(sel_per[a],3) for a in ACTIONS})})")

            # ---- TRAIN+TEST embeddings = the selected checkpoint's CACHED dirs ----
            emb_dirs = {a: cache[(sel_ep, a)] for a in ACTIONS}
