...
def get_directed_alpha(key_left, key_right, triplet_modules):
    """Directed alpha-as-margin for a NON-NEUTRAL effort pair, read from the module's
    alpha_dataframes (the same authentic source the frozen-encoder triplet trainer uses):
      alpha_0_2 = c(0,2) - c(0,1)  (Left-Right margin, Left anchor)
      alpha_2_0 = c(0,2) - c(1,2)  (Left-Right margin, Right anchor)
    Returns (alpha_left, alpha_right) oriented to (key_left, key_right), or None if unrated /
    neutral-involving / pair absent. Mirrors get_inverse_direct_comparison_value's lookup."""
    if not isinstance(triplet_modules, list):
        triplet_modules = [triplet_modules]
    ...
    for module in triplet_modules:
        ...
        if pair.empty:
            return None
        row = pair.iloc[0]
        stored = list(row["efforts_tuples"])          # (stored_left, stored_right)
        a02, a20 = float(row["alpha_0_2"]), float(row["alpha_2_0"])
        # orient stored (left=0,right=2) alphas onto our (key_left, key_right) ordering
        if stored[0] == effort1:
            return (a02, a20)                          # our left == stored left
        return (a20, a02)                              # flipped
    return None
...
ENCODERS = {
    ...
    "MAMP+pose": dict(family="mamp",    ckpt_dir="MAMP/output_dir/lma_mamp_pose_holdout",
                      config="config/lma_mamp_pretrain.yaml", stride_default=3),
    # NOTE: AE-vel / AE-recon below are the ORIGINAL vanilla runs whose pose-reconstruction and
    # velocity losses did NOT cleanly separate (weighted/entangled). They are SUPERSEDED for the
    # velocity-vs-pose factorial by the vanilla-rot* unit-weight cells further down. Keep them
    # only for back-reference; do NOT use them in the clean factorial (use vanilla-rot* instead).
    ...
    "MLD-VAE-holdout": dict(family="mld", kind="vae",
                            ...
                            n_windows=1, stride_default=3),
    # --- CLEAN single-variable velocity-vs-pose factorial (unit 0/1 weights) -----------------
    # These REPLACE AE-vel/AE-recon for the factorial: each isolates exactly one loss term on
    # (weight 1) vs off (weight 0), so pose-reconstruction and velocity are cleanly separated.
    #   rot1_vel0 = reconstruction only | rot1_vel1 = recon + velocity | rot0_vel1 = velocity only
    # MLD-AE backbone velocity-only cell (completes the MLD side of the same factorial).
    "AE-velonly":       dict(family="mld", kind="ae",
                             ckpt_dir="checkpoints_ae_mld/v0_ae_velonly_3ds_seed0",
                             n_windows=1, stride_default=3),
    "vanilla-rot1vel0": dict(family="vanilla", ckpt_dir="checkpoints/v3_ablation_rot1_vel0",
                             n_windows=1, stride_default=1),
    "vanilla-rot1vel1": dict(family="vanilla", ckpt_dir="checkpoints/v3_ablation_rot1_vel1",
                             n_windows=1, stride_default=1),
    "vanilla-rot0vel1": dict(family="vanilla", ckpt_dir="checkpoints/v3_ablation_rot0_vel1",
                             n_windows=1, stride_default=1),
    # 6-dataset MAMP+pose: the corpus-MATCHED comparator for the TMR learned-baseline comparison
    # (NOT the paper headline, which stays legacy-3). Currently @599 epochs (~half the ~1200
    # convergence) -- PSP profiling will confirm whether it needs resume-training.
    "MAMP+pose-6ds":    dict(family="mamp", ckpt_dir="MAMP/output_dir/lma_mamp_pose_holdout_6ds",
                             config="config/lma_mamp_pretrain.yaml", stride_default=3),
}
